 ...
 * 'Node' represent each node having childrens.
 * 'connect' represent each edge connecting two Nodes.
 */

struct _node {
	int distance; // distance from the first node
	struct _connect *children[100]; // going to nodes
...
int getMinDistanceNode(struct _connect *array[], int size){
...
int main(int argc, char *argv[]){
	...
	int nodesNo = 0;
	int edgesNo = 0;
	int connectCtr = 0; // count the acnes
	int previous = 0;
	...
	struct _connect connect[edgesNo];
	...
	while (fscanf(ifp, "%d %d %d", &from, &to, &distance) == 3) { /* read from file */
		...
		connect[connectCtr].from = from;
		connect[connectCtr].to = to;
		connect[connectCtr].value = distance;
		connect[connectCtr].edge = &nodes[to];

		nodes[from].children[childsCtr] = &connect[connectCtr];
